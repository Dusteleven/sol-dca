...
async function waitForConfirmation(connection, signature, maxWaitMs = 60000, // 1 minute timeout
pollInterval = 2000) {
    ...
    throw new Error(`Transaction ${signature} not confirmed within ${maxWaitMs / 1000}s`);
}
export async function startIncrementalSell({ connection, wallet, prudentReserveSOL = 0.1, }) {
    const logPath = path.resolve(__dirname, `../logs/${wallet.publicKey.toBase58()}.log`);
    const log = (msg) => {
    ...
    while (true) {
        try {
            ...
            const randomSellSOL = Math.min(randomInRange(0.1, 0.2), excessLamports / LAMPORTS_PER_SOL);
            const sellAmountLamports = Math.floor(randomSellSOL * LAMPORTS_PER_SOL);
            log(`🟢 Attempting to sell ${randomSellSOL.toFixed(9)} SOL`);
            // 1. Quote
            const quoteRes = await fetch(`${JUPITER_QUOTE_URL}?inputMint=${SOL_MINT}&outputMint=${USDC_MINT}&amount=${sellAmountLamports}&slippageBps=50&restrictIntermediateTokens=true`);
